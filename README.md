...
NextUSinger Studio 是一个面向歌唱合成爱好者/开发者的开源原型：它提供类似 Synthesizer V 的本地编辑器思路，支持导入 DiffSinger/OpenVPI 风格声库目录，并通过可插拔后端进行歌唱合成、音频导出和 AI 辅助创作。
...
- **前端雏形**：React + Vite，本地网页工作站，可导入声库路径、编辑音符、调用 AI、导出 WAV。
- **测试**：包含 voicebank 扫描与 AI 调音单元测试。
